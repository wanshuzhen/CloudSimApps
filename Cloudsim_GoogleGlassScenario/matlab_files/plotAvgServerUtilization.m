function [] = plotAvgServerUtilization()
     ...
     for s=1:numOfSimulations
         for i=1:size(vmType,2)
             for j=startOfMobileDeviceLoop:stepOfMobileDeviceLoop:endOfMobileDeviceLoop
                 filePathCreated = strcat(filePath,'SIMRESULT_ITE',int2str(s),'_',char(vmType(i)),'_MEAN20_',int2str(j),'DEVICE_VM_LOAD.log');
                 readData = dlmread(filePathCreated,';',1,0);

                 results(i,j) = results(i,j) + mean(readData(:,2));
             end
         end
     end
    ...
    ylabel('Average Number of Tasks Running on VM');
    %set(gca,'YLim',[0 1]);
end